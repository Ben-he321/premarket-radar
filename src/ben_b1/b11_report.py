...
原有关联项目Alpaca凭证实际有效，本轮继续SIP/Basic，无IEX、Futu、yfinance或合成研究数据回退。取得 **3,043,187条真实分钟记录**；15只证券各有145—148日入场前常规盘预热及固定区间174交易日数据，重复时间戳、无效OHLC、缺失OHLC检查均0。日线raw/split/all分开保存，官方收盘与末一分钟收盘分开；收盘竞价的价格/量条件另附核对。
...
完整事件内核已接入冻结信号、跨日交易、分档/移动止损、15:50因果数据、版本化财报退出、报价数量去重、部分成交净7%/2R复算、现金/预留/结算、公司行动、P200真实负债计息与保证金假设、原子恢复。**{integration['tests']}项工程测试通过，失败0**；测试数据只在临时目录，不进入研究目录。Windows文件暂时占用修复有界重试；持续失败保留旧检查点。所有已知开发失败及恢复版本保留在ERROR_AND_REPAIR_LOG。
