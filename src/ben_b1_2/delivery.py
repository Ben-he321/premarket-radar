...
def report(sample_version,portfolio_version):
    ...
    if partials:
        lines+=['','### 未完成账户的已有断点证据','',
          '以下财务字段提取操作仅复制已保存检查点，没有恢复引擎、执行事件或核验完整数据库；后续独立副本恢复检查另列。中途现金不是期末权益，部分成交记录不是完整季度回测。尚未完整走完季度时，季度收益和回撤保持未知；已保存的季度快照按实际证据单列。','',
          '| 账户 | 断点事件截止 | 已证明完成的纽约交易日 | 中途现金 | 该断点买/卖成交笔数 | 提取操作中的数据库核验 |',
          '|---|---|---|---:|---|---|']
        for r in partials:
        ...
        partial_audit=existing(ROOT/'engineering/DELIVERY_VALIDATION_PARTIAL_Q1_B.json',{})
        if partial_audit:
            lines+=['',f'已保存部分的财务核对：{partial_audit.get("status","NOT_VERIFIED")}。详见 engineering/DELIVERY_VALIDATION_PARTIAL_Q1_B.md；它只验证已经落盘的现金流、股数、费用与时序，不证明未运行日期的结果。']
        isolated_restore=existing(ROOT/'engineering/PARTIAL_ISOLATED_RESTORE.json',{})
        if isolated_restore:
            lines+=['',f'隔离副本恢复检查：{isolated_restore.get("status","NOT_VERIFIED")}。详见 engineering/PARTIAL_ISOLATED_RESTORE.json。只针对已保存前缀，不执行新事件，不等于季度完成，也不替代四本完整恢复验收。原始提取清单中的 NOT_PERFORMED 仍描述提取操作本身。']
    lines+=['','## 同期SPY/QQQ参照','',
    ...
    lines+=['','## 数据覆盖与验证边界','',
      ...
      f'初始历史输入质量检查覆盖{quality["checked_objects"]}个对象、{quality["rows"]:,}行：重复{quality["duplicates"]}、无效OHLC {quality["invalid_ohlc"]}、缺失成交量{quality["missing_volume"]}。此项不代表随后全部持仓报价已完成同一项检查；报价请求和错误另见各账户覆盖记录。空响应、分钟无成交、停牌与接口失败不互相替代。',
      ...
      '## 结论与下一步','',
      '本轮交付应区分：有真实模型成交的工程回放；全部条件不满足而零成交；数据未知导致无法评价；以及因错误/资源边界未完成的账户。具体状态和错误不以空表或初始化现金代替。',
...
def package(sample_version,portfolio_version):
    ...
    engineering_names={'ENGINE_INTEGRATION.json','ENGINE_INTEGRATION.md','B12_ENGINE_INTEGRATION.json','BENCHMARK_IMPLEMENTATION_CHECK.json',
        ...
        'DELIVERY_VALIDATION_PARTIAL_Q1_B.json','DELIVERY_VALIDATION_PARTIAL_Q1_B.md','validate_partial_q1_b.py',
        'delivery_validation_partial_q1_b.log','delivery_validation_final.log','FINAL_VALIDATION_PRESERVATION.json',
        'DELIVERY_VALIDATION_BEFORE_FINAL_AGGREGATE.json','partial_actual_extraction.log',
        'PARTIAL_ISOLATED_RESTORE.json','PARTIAL_ISOLATED_RESTORE.md','PARTIAL_ISOLATED_RESTORE.log','PARTIAL_ISOLATED_RESTORE.py'}
    for stem in ['all_b1_b12','all_b1_b12_compact','b11_regression','compact_initial','compact_expanded','compact_final',
        ...
        'delivery_validation_samples40','benchmarks_real_run','delivery_activity_regression','delivery_validation_A_stage','delivery_partial_regression','delivery_validation_Q0_B_stage','delivery_interim_valuation_regression','delivery_unknown_boundary_regression','delivery_mixed_timestamp_regression','delivery_shared_evidence_regression','delivery_final_narrative_regression']:
