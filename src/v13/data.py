...
def complex_actions():
    ...
    for rec in read(root()/'input_snapshot.json')['universe']['records']:
        s=rec['symbol']
        for kind,records in read(source()/'actions'/f'{s}.json',{}).get('data',{}).items():
            ...
            for a in records:
                ...
                unchanged=a.get('acquirer_symbol')==s and a.get('acquiree_symbol')!=s
                result.setdefault(day,[]).append({'symbol':s,'kind':kind,'day':day,'id':a.get('id','UNKNOWN'),
                                                  'status':'ACQUIRER_SHARES_UNCHANGED' if unchanged else 'UNVERIFIED_COMPLEX_ACTION'})
    return result
