...
import pandas as pd
from .runtime import *
from src.v13.statistics import summary, by_adjust
...
def run():
    ...
    raw_adjusted=by_adjust([x['p'] if x['p'] is not None else 1. for x in primary])
    for row,q in zip(primary,raw_adjusted):row['raw_q_by']=float(q)
